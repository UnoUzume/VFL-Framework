...
该模块提供了 Imagenette 数据集的加载、预处理和分发功能，
用于联邦学习场景下的图像分类任务。
"""
...
class BaseDataset(Dataset[de.TUImageSample]):
	"""Imagenette 数据集的基本加载类。

	继承自 `Dataset` 类，用于加载和预处理 Imagenette 数据集。
	"""
...
def splitImage(data: de.TUImages, nParty: int) -> list[de.TUImages]:
	...
	if nParty == 3:
		return [data[..., :53], data[..., 53:106], data[..., 106:]]
	if nParty == 4:
		return [
			data[..., :80, :80],  # 左上角
			data[..., :80, 80:],  # 右上角
...
class Handler(DataHandler):
	...
	def prepare(self, dpData: Path) -> None:
		...
		Imagenette(dpData, 'val', '160px', download=True)
