...
class VillainCb(VFLCallback):
	...
	def createEps(self, embeds: tc.Tensor, beta: float = 0.4, isAugment: bool = False) -> tc.Tensor:
		"""生成用于后门攻击的触发器向量。
	...
	def onTrainBtmOut(self, m: BaseVFLArch, v: StepVars) -> None:
		...
		if len(self.aDstPos) > 0:  # 如果找到投毒目标
			v.lBtmOut[0][self.aDstPos] += self.createEps(v.lBtmOut[0].detach(), 1, True)
	...
	def onValBtmOut(self, m: BaseVFLArch, d: dict[str, StepVars]) -> None:
		v = d['Attack']
		v.lBtmOut[0] += self.createEps(v.lBtmOut[0].detach(), 2)
